...
REDIS_KEY_QUEUE = "news:money163:urls"          # 待处理队列 (List)
REDIS_KEY_VISITED = "news:money163:visited"    # 已去重集合 (Set)
REDIS_KEY_RESULT = "news:money163:details"     # 最终结果存储 (Hash)
...
class HuXiuSpider:
    ...
    async def parse_main_page(self, page):
        ...
        抓取首屏已有数据
        """
        items = await page.locator(".channel_news_item").all()
        for item in items:
    ...
    async def fetch_details(self, browser):
        ...
        while True:
            ...
            try:
                result = {"title": "", "url": url, "data": {
                    "title": "", "url": url, "author": "", "summary": "", "pic_path": ""}}
    ...
    async def run(self):
        async with async_playwright() as p:
            ...
            # 2. 访问主页并解析首屏
            print("正在打开主页...")
            await page.goto(f"https://money.163.com/", wait_until="networkidle", timeout=30000)
