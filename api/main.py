...
REDIS_KEY_RESULT = "huxiu:clean_data"

# 默认分类
DEFAULT_CATEGORY = "huxiu:clean_data"
...
async def lynx_index(
    cat: str = Query(DEFAULT_CATEGORY, description="分类Key"),
    page: int = Query(1, ge=1),
    size: int = Query(30, ge=1)
):
    # 1. 获取该分类下所有的 Key
    all_keys = r.hkeys(cat)
    total = len(all_keys)
    ...
    articles = {}
    if target_keys:
        values = r.hmget(cat, target_keys)
        articles = dict(zip(target_keys, values))

    # 4. 构建 HTML
    html = f"""
    <html>
    <head><title>终端资讯站 - {cat}</title></head>
    <body bgcolor="#eeeeee">
        <h1>[ 资讯中心 ]</h1>
        <p>
            <b>分类：</b> 
        ...
        <hr>
        <h3>当前列表 ({start+1}-{min(end, total)} / 共{total}条)</h3>
        <ul>
    """

    for url, data_str in articles.items():
        data = json.loads(data_str)
        article_id = get_url_id(url)
        # 这里的链接带上 cat 参数，防止查看详情回来时分类丢失
        html += f'<li><a href="/view/{article_id}?cat={cat}">{data.get("title")}</a></li>\n'

    html += "</ul><hr><p>"

    # 5. 上下页逻辑
    if page > 1:
        html += f'<a href="/?cat={cat}&page={page-1}">[ 上一页 ]</a>  '
    if end < total:
        html += f'<a href="/?cat={cat}&page={page+1}">[ 下一页 ]</a>'
...
async def lynx_detail(article_id: str, cat: str = DEFAULT_CATEGORY):
    all_articles = r.hgetall(cat)
    target_data = None
    for url, data_str in all_articles.items():
        if get_url_id(url) == article_id:
            target_data = json.loads(data_str)
            break
